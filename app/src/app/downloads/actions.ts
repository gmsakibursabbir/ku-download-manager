...
export function primaryAction(d: Download): { icon: LucideIcon; label: string; run: () => void } | null {
  switch (d.status) {
    ...
    case "error":
      return { icon: RotateCcw, label: t("Retry"), run: () => void run(api.resume([d.id]), "Could not retry") };
    case "completed":
      return { icon: FolderOpen, label: t("Show in folder"), run: () => void run(api.openFolder(d.id), "Could not open the folder") };
  }
}
...
export function contextMenu(ids: string[], opts: { confirmRemove: (ids: string[]) => void; onVerify: (id: string) => void }): MenuItem[] {
  ...
  if (one) {
    items.push(
      { label: t("Open"), icon: ExternalLink, shortcut: "Enter", disabled: !finished, onSelect: () => openDownload(one) },
      { label: t("Show in folder"), icon: FolderOpen, shortcut: "Ctrl Shift O", onSelect: () => void run(api.openFolder(one.id), "Could not open the folder") },
      "sep",
    );
  }
  ...
  if (anyPause) items.push({ label: t("Pause"), icon: Pause, shortcut: "Space", onSelect: () => void run(api.pause(ids), "Could not pause") });
  const again = list.filter((d) => (d.status === "completed" || d.status === "error") && d.kind !== "torrent" && d.kind !== "magnet");
  if (again.length) items.push({ label: again.length > 1 ? `Download ${again.length} again` : "Download again", icon: RotateCcw, onSelect: () => void run(api.redownload(again.map((d) => d.id)), "Could not download again") });
  items.push({
    label: t("Move to queue"),
    icon: ListOrdered,
    submenu: [
      { label: t("No queue (start directly)"), checked: list.every((d) => !d.queueId), onSelect: () => void run(api.moveToQueue(ids, null), "Could not move") },
      "sep",
  ...
  if (one && inQueue && !finished) {
    items.push(
      { label: t("Move to top"), icon: ArrowUpToLine, onSelect: () => void run(api.reorder(one.id, "top"), "Could not reorder") },
      { label: t("Move up"), icon: ArrowUp, shortcut: "Alt ↑", onSelect: () => void run(api.reorder(one.id, "up"), "Could not reorder") },
      { label: t("Move down"), icon: ArrowDown, shortcut: "Alt ↓", onSelect: () => void run(api.reorder(one.id, "down"), "Could not reorder") },
      { label: t("Move to bottom"), icon: ArrowDownToLine, onSelect: () => void run(api.reorder(one.id, "bottom"), "Could not reorder") },
    );
  }
  items.push("sep");
  if (one) {
